...
use crate::{diagnostic_shim::*, resolved_at_shim::*};
...
impl MetaItem {
    ...
    pub fn required_nested_item(&self, name: &str) -> Result<Self, Diagnostic> {
        self.nested_item(name)?
            .ok_or_else(|| self.span().error(format!("Missing required option `{}`", name)))
    }
    ...
    pub fn expect_bool_value(&self) -> bool {
        match self.str_value().as_ref().map(String::as_str) {
            ...
            _ => {
                self.span()
                    .error(format!("`{0}` must be in the form `{0} = \"true\"`", self.name()))
                    .emit();
                false
            }
        }
    ...
    pub fn word(&self) -> Result<syn::Ident, Diagnostic> {
        ...
        match self.meta {
            ...
            _ => {
                ...
                Err(self
                    .span()
                    .error(format!("Expected `{}` found `{}`", self.name(), quote!(#meta))))
            }
        }
    }
